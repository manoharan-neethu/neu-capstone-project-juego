...
class challenge:
    ...
    def append_to_docker_compose(user_id,user_flag,rand_num):
        ...
        file = Path(dest)
        data = file.read_text().replace('{id}','0'+str(user_id)).replace('{key}',str(rand_num)).replace('{flag}',str(user_flag)+'}')  
    ...
    def create_user_files(user_id,user_flag,rand_num):
        ...
        challenge.append_to_docker_compose(user_id,user_flag,rand_num)
    ...
    def create_flags(users:int,flag:str): 
        ...
        for n in range(1,users+1):
                ...
                uf.write(user_flag + '}'  + "\n")
                challenge.create_user_files(n,user_flag,random_number)
                print("--------------------------------------------------------------") 
